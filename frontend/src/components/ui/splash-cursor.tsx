...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const footer = document.querySelector('footer');
      ...
      window.addEventListener('mousemove', updateCanvasVisibility);
      // Set visible by default
      if (canvas) canvas.style.opacity = '1';
      return () => {
        window.removeEventListener('mousemove', updateCanvasVisibility);
      };
  }, []);
  return (
    <div className="fixed top-0 left-0 z-10 pointer-events-none">
      <canvas ref={canvasRef} id="fluid" style={{width: '100vw', height: 'calc(100vh - 80px)'}} />
    </div>
  );
}
